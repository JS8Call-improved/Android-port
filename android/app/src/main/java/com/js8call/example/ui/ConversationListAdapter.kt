...
class ConversationListAdapter : ListAdapter<ConversationSummary, ConversationListAdapter.ConversationViewHolder>(
    ...
    class ConversationViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(conversation: ConversationSummary) {
            // Set callsign
            callsignText.text = conversation.callsign

            // Set avatar (first letter of callsign, or @ for groups)
            avatarText.text = if (conversation.callsign.startsWith("@")) {
                "@"
            } else {
                conversation.callsign.firstOrNull()?.uppercase() ?: "?"
            }
